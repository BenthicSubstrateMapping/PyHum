...
if __name__ == '__main__': 
    ...
    try:
       opts, args = getopt.getopt(argv,"hi:s:c:t:f:W:b:p:")
    except getopt.GetoptError:
         print 'pyhum_correct.py -i <humfile> -s sonpath [[-c <speed sound water (m/s)> -t <transducer length (m)> -f <frequency (kHz)> -W <max transducer power (watts)> -b bed pick <1 = auto, 2 = manual> ]]'
         sys.exit(2)
    for opt, arg in opts:
       if opt == '-h':
         print 'pyhum_correct.py -i <humfile> -s sonpath [[-c <speed sound water (m/s)> -t <transducer length (m)> -f <frequency (kHz)> -W <max transducer power (watts)> -b bed pick <1 = auto, 2 = manual>]]'
    ...
    es = runningMeanFast(e,20)
    ns = runningMeanFast(n,20)
    ...
    if bedpick == 1: # auto
       ...
       if doplot==1:
          ...
          if ny>10000:
             Z,inds = sliding_window(data_port,(nx,10000))
             del inds

          nx, ny = np.shape(data_star)
          if ny>10000:
             Zstar,inds = sliding_window(data_star,(nx,10000))
             del inds

          if ny>10000:
             Zx,inds = sliding_window(np.squeeze(x),(10000))
             del inds

          ny = np.shape(bed)[0]
          if ny>10000:   
             Zbed,inds = sliding_window(np.squeeze(bed),(10000))
             del inds

          if ny>10000:   
             Zdist,inds = sliding_window(np.squeeze(dist_mi),(10000))
             del inds   
          del ny
          ...
          if 'Z' in locals():
             ...
             plt.ylim(10,0)
             plt.ylabel('Range (m)'); plt.xlabel('Distance along track (m)')
             custom_save(sonpath,'bed_2picks')
             del fig
    ...
    if doplot==1:
       ...
       dat = ppdrc.ppdrc(merge, 768)
       merge2 = rescale(dat.getdata(),0,255)
       del dat
       del port_mg_la, star_mg_la

       nx, ny = np.shape(merge)
       if ny>10000:
          Z,inds = sliding_window(merge2,(nx,10000))
          del nx, ny, inds
          Zdist,inds = sliding_window(np.squeeze(dist_m),(10000))
          del inds 
          flag=1
       else:
          Z = merge2.copy()  
       ...
       if flag==1:
          for k in xrange(len(Z)):
             ...
             plt.axis('normal'); plt.axis('tight')
             custom_save(sonpath,'merge_corrected_scan'+str(k))
             del fig
          ...
          plt.ylabel('Range (m)'), plt.xlabel('Distance along track (m)')
          plt.axis('normal'); plt.axis('tight')
          custom_save(sonpath,'merge_corrected_scan'+str(k+1))
          del fig
          ...
          plt.axis('normal'); plt.axis('tight')
          custom_save(sonpath,'merge_corrected_scan')
          del fig   
       del Z, Zdist
    ...
    del data_dwnlow_dB, data_dwnhi_dB
    ...
    del beam_av_dwnhi, beam_av_dwnlow, ref_dwnlow, ref_dwnhi, cf
    ...
    if doplot==1:
       ...
       if ny>10000:
          Z,inds = sliding_window(dwnlow_mg_la,(nx,10000))
          del nx, ny, inds
          Zdist,inds = sliding_window(np.squeeze(dist_m),(10000))
          del inds 
       ...
       if flag==1:
          for k in xrange(len(Z)):
             ...
             plt.axis('normal'); plt.axis('tight')
             custom_save(sonpath,'dwnlow_corrected_scan'+str(k))
             del fig
          ...
          plt.ylabel('Range (m)'), plt.xlabel('Distance along track (m)')
          plt.axis('normal'); plt.axis('tight')
          custom_save(sonpath,'dwnlow_corrected_scan'+str(k+1))
          del fig
          ...
          plt.axis('normal'); plt.axis('tight')
          custom_save(sonpath,'dwnlow_corrected_scan')
          del fig   
       del Z, Zdist
       ...
       if ny>10000:
          Z,inds = sliding_window(dwnhi_mg_la,(nx,10000))
          del nx, ny, inds
          Zdist,inds = sliding_window(np.squeeze(dist_m),(10000))
          del inds 
       ...
       if flag==1:
          for k in xrange(len(Z)):
             ...
             plt.axis('normal'); plt.axis('tight')
             custom_save(sonpath,'dwnhi_corrected_scan'+str(k))
             del fig
          ...
          plt.ylabel('Range (m)'), plt.xlabel('Distance along track (m)')
          plt.axis('normal'); plt.axis('tight')
          custom_save(sonpath,'dwnhi_corrected_scan'+str(k+1))
          del fig
          ...
          plt.axis('normal'); plt.axis('tight')
          custom_save(sonpath,'dwnhi_corrected_scan')
          del fig   
       del Z, Zdist
